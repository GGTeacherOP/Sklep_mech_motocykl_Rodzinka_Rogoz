...
if ($is_editing) {
    ...
    $images_query = "SELECT * FROM product_images WHERE product_id = ? ORDER BY position ASC";
    ...
    if (empty($name) || empty($slug) || empty($description) || $price <= 0) {
        ...
        if ($is_editing) {
            ...
            $stmt->bind_param("sssddiiiisi", 
                ...
                $stock_quantity, $category_id, $brand_id, $featured, $status, $product_id
            );
            ...
            $insert_query = "INSERT INTO products (name, slug, description, price, sale_price, stock_quantity, 
            ...
            if (!$stmt) {
                ...
                $stmt->bind_param("sssddiiiis", 
                    $name, $slug, $description, $price, $sale_price, 
                    $stock_quantity, $category_id, $brand_id, $featured, $status
                );
                ...
                if ($file['error'] === 0) {
                    ...
                    if (in_array($file_type, $allowed_types)) {
                        if (move_uploaded_file($file['tmp_name'], $target_file)) {
                            ...
                            $position_query = "SELECT MAX(position) AS max_position FROM product_images WHERE product_id = ?";
                            $stmt = $conn->prepare($position_query);
                            $stmt->bind_param("i", $product_id);
                            $stmt->execute();
                            $position_result = $stmt->get_result()->fetch_assoc();
                            $position = ($position_result['max_position'] ?? 0) + 1;
                            
                            // Dodawanie zdjęcia do bazy danych
                            $image_query = "INSERT INTO product_images (product_id, image_path, position, created_at)
                                           VALUES (?, ?, ?, NOW())";
                            $stmt = $conn->prepare($image_query);
                            $relative_path = 'uploads/products/' . $file_name;
                            $stmt->bind_param("isi", $product_id, $relative_path, $position);
                            $stmt->execute();
...
if (isset($_GET['action']) && $_GET['action'] === 'delete_image' && isset($_GET['image_id'])) {
    ...
    if ($image_result && $image_result->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            // Aktualizacja pozycji pozostałych zdjęć
            $update_position_query = "UPDATE product_images SET position = position - 1 WHERE product_id = ? AND position > ?";
            $stmt = $conn->prepare($update_position_query);
            $stmt->bind_param("ii", $product_id, $image['position']);
            $stmt->execute();
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            <label for="stock_quantity" class="block text-sm font-medium text-gray-700 mb-1">Stan magazynowy</label>
